...
| ----- | ---------- | ------------------------------------------------------------ |
| 1.3.3 | 2026-09-30 | **New function：**<br />1.Supports new device: Wireless Repeater.<br />2.Support multiple languages: Maltese, Slovenian, Albanian, Montenegrin and Hebrew.<br />3.Supports APN settings during cellular network provisioning.<br />4.Add configuration item for canceling panic button alarm.<br /> |
...
| **Settings**            | /    | Only the camera super administrator can modify the hub settings.This is where you can pair your camera to the Hub.|
...
|    **Sub-Device Type**          |![transmit-setting-type](https://dusunprj.oss-us-west-1.aliyuncs.com/RBGW/pic/transmitter/transmit-type-list.jpg) | **Intrusion**：Trigger an alarm on motion, door/window opening, or activation of other detectors.<br />**Fire**：Generate an alarm when fire detectors are activated.<br />**Auxiliary alarm**：<br />Trigger an alarm when the auxiliary request button is pressed.<br />**Panic button**：Activate an alarm upon pressing the panic button.<br />**Gas alarm**：Alarm when gas concentration is exceeded.<br />**Leakage**：Alarm triggered by water leakage detectors.<br />**Glass break**：Alarm triggered by the glass break sensor.<br />**High temperature**：Trigger an alarm when the upper temperature limit is exceeded.<br />**Low temperature**：Activate an alarm when the lower temperature limit is breached.<br />**Masking**：Alarm when the device masking is detected.<br />**Duress code (opening)**：Alarm upon entry of duress code.<br />**Vibration**：Alarm when the vibration sensor is activated.This event type is only available under Pulse mode.<br />**Custom**：User-configurable event type. This event is not transmitted to the security company's monitoring station.<br /> |
